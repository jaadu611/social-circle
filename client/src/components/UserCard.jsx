...
import { MapPin, PlusCircle, UserCheck, UserPlus } from "lucide-react";
...
const UserCard = ({ user }) => {
  ...
  const handleAction = async (url, optimisticState, revertState) => {
  ...
  const handleFollow = () => handleAction("/api/user/follow", true, false);
  const handleUnfollow = () => handleAction("/api/user/unfollow", false, true);
        ...
        >
          {isConnected ? (
            <>
              ...
              <span className="text-[10px] xs:text-xs sm:text-sm truncate leading-none">
                Message
              </span>
            </>
          ) : (
